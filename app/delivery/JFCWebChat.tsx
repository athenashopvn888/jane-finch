...
const SESSION_KEY = "sod-web-chat:JFC";
...
export default function JFCWebChat() {
  ...
  useEffect(() => {
    const initial = window.setTimeout(() => {
      if (token) void refresh(token).catch(() => setNotice("Your previous JFC chat could not be restored."));
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      if (!document.hidden) void refresh(token).catch(() => undefined);
    }, 15000);
  ...
  async function start(event: FormEvent) {
    ...
    try {
      const data = await payload(await fetch(`${API_BASE}/api/web-chat/session`, {
        ...
        body: JSON.stringify({ storeId: "JFC", customerName: intent === "NEW_CUSTOMER" ? name : "", phone, intent, message: firstMessage, smsConsent }),
      }));
      localStorage.setItem(SESSION_KEY, data.token);
      setToken(data.token);
      setConversation(data.conversation);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Web Chat is unavailable.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function uploadPhoto(file?: File) {
    ...
    try {
      ...
      const data = await payload(await fetch(`${API_BASE}/api/web-chat/id-review`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        body: JSON.stringify({ mimeType: "image/jpeg", imageBase64: await readBase64(photo) }),
      }));
  ...
  return (
    <aside className={`sod-web-chat ${open ? "open" : ""}`} aria-label="JFC Web Chat">
      ...
      {open && <section className="sod-chat-panel" role="dialog" aria-modal="true" aria-label="JFC Web Chat">
      <header><div><strong>JFC Web Chat</strong><small>Start your delivery order with the Jane Finch Cannabis dispatcher</small></div><button type="button" onClick={() => setOpen(false)} aria-label="Minimize chat">×</button></header>
      ...
      {!token ? (!availability || availability.state !== "AVAILABLE" ? <div className="sod-chat-start sod-chat-paused">
        ...
        <label className="sod-sms-consent"><input required type="checkbox" checked={smsConsent} onChange={(event) => setSmsConsent(event.target.checked)} /><span>I agree to receive one text from Jane Finch Cannabis to confirm this mobile number for my Web Chat. Reply YES to confirm. Message and data rates may apply.</span></label>
        ...
        <div className="sod-chat-scroll">
          ...
          {retryNeeded && !retryExpanded && <div className="sod-id-retry-prompt" role="status">
            ...
            <button type="button" onClick={() => setRetryExpanded(true)}>Try ID photo again</button>
          </div>}
          {showUpload && <section className="sod-id-upload">
            <strong>New customer ID verification</strong>
            <p>Take one clear selfie while holding your government-issued photo ID beside your face.</p>
