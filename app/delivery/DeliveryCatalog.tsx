...
function ProductPricing({ product }: { product: Product }) {
  ...
  return <div className="product-pricing">
    {compact.length > 0 && <div><div className="compact-price-grid">{compact.map((option) => <div className="compact-price" key={option.key}><span>{option.label}</span><strong>{formatCurrency(option.price)}</strong></div>)}</div></div>}
    {(regular28 || member || bundles.length > 0) && <div className="decision-prices">
      ...
      {bundles.map((offer, index) => { const quantity = Number(offer.quantity); const total = Number(offer.totalPrice); const each = get28gBundleEachDisplayPrice(quantity, total, offer.perUnitPrice); return <div className="decision-tile bundle-decision" key={`${quantity}-${index}`}><span>{quantity} × 28g DEAL</span><div className="bundle-numbers"><strong>{formatCurrency(each)} <small>each</small></strong><b>{formatCurrency(total)} <small>total</small></b></div></div>; })}
